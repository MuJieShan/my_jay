...
def main():
    ...
    # data pruning
    compress = config.reg
    loss_gap_before_file = f"/clzs_test003/my_jay/roberta/score/psp-zero/{task}_before.pt"
    loss_gap_before = torch.load(loss_gap_before_file)
    loss_gap_after_file = f"/clzs_test003/my_jay/roberta/score/psp-zero/{task}_after.pt"
    loss_gap_after = torch.load(loss_gap_after_file)
